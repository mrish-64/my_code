...
def get_best_nest(nest, newnest, fitness, n, dim,dc,str_with_copy):
    ...
    for j in range(0, n):
        ...
        temp_position = (sigV > numpy.random.rand(
            newnest[j, :].size)).astype(int)
        newnest[j, :] = temp_position
        ###########################################
        tmp=numpy.array(str_with_copy)
        numpy.put(tmp,dc,newnest[j,:])
        ############################################
        fnew = functions.f(tmp.astype(int))
        if fnew <= fitness[j]:
            fitness[j] = fnew
...
def opt(dc, str_with_copies):
    ...
    n = 50
    # N_IterTotal=1000
    N_IterTotal = 100 #also test 200 for better performance
    ...
    fmin, bestnest, nest, fitness = get_best_nest(nest, new_nest, fitness, n, dim,dc,str_with_copies)
    convergence = []
    # Main loop counter
    for iter in range(0, N_IterTotal):
        ...
        fnew, best, nest, fitness = get_best_nest(nest, new_nest, fitness, n, dim,dc,str_with_copies)
