...
def opt(dc,str_with_copies):
   ...
   while t < Max_iter:
      for i in range(0, SearchAgents_no):
         ...
         for j in range(dim):
               ...
               numpy.put(temp_x,dc,temp_position)
               ################################
               fitness = functions.f(temp_x.astype(int))
      ...
      for i in range(0, SearchAgents_no):
         ...
         for j in range(0, dim):
               ...
               if p < 0.5:
                  ...
                  # Eq. (2.5)
                  Positions[i, j] = distance2Leader * \
                     math.exp(b*l)*math.cos(l*2*math.pi)+Leader_pos[j]

      convergence_curve[t] = Leader_score
      # if (t % 1 == 0):
      #    print(['At iteration ' + str(t) +
      #          ' the best fitness is ' + str(Leader_score)])
      t = t+1
   ...
   numpy.put(temp,dc,Leader_pos)
   temp_lst=temp.tolist()
   ###############################
   x_star="".join(temp_lst)
   return [Leader_score,x_star]
