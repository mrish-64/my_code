...
def opt(dc,str_with_copies):
    ...
    while t < Max_iter:
        for i in range(0, SearchAgents_no):
            ...
            temp_position =( sigV > numpy.random.rand(X[i,:].size)).astype(int)
            X[i,:]=temp_position
            ###########################################
            tmp=numpy.array(str_with_copies)
            numpy.put(tmp,dc,X[i,:])
            ############################################
            # fitness of locations
            fitness = functions.f(tmp.astype(int))
        ...
        for i in range(0, SearchAgents_no):
            ...
            # Harris' hawks perch randomly based on 2 strategy:
                ...
                # Soft besiege Eq. (10) in cited cited paper
                if r < 0.5 and abs(Escaping_Energy) >= 0.5:
                    ...
                    temp_position =( sigV > numpy.random.rand(X1.size)).astype(int)
                    X1=temp_position
                    ###########################################
                    tmp=numpy.array(str_with_copies)
                    numpy.put(tmp,dc,X1)
                    ############################################
                    if functions.f(tmp.astype(int)) < fitness:  # improved move?
                        X[i, :] = X1.copy()
                        ...
                        temp_position =( sigV > numpy.random.rand(X2.size)).astype(int)                    
                        X2=temp_position
                        ###########################################
                        tmp=numpy.array(str_with_copies)
                        numpy.put(tmp,dc,X2)
                        ...
                        if functions.f(tmp.astype(int)) < fitness:
                            X[i, :] = X2.copy()
    ...
    numpy.put(temp,dc,Rabbit_Location)
    temp_lst=temp.tolist()
    ###############################
    x_star="".join(temp_lst)
    return [Rabbit_Energy,x_star]